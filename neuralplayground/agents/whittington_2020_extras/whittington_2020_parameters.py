...
def parameters(n_td_reward=1):
    ...
    # Number of available actions, excluding the stand still action (since standing
    # still has an action vector
    #   full of zeros, it won't add to the action vector dimension)
    params["n_actions"] = 4
    # Bias for explorative behaviour to pick the same action again, to encourage
    # straight walks
    ...
    params["shiny_rate"] = 0
    # Discount factor in calculating Q-values to generate shiny object oriented
    # behaviour
    ...
    # Maximum length of a walk on one environment. Walk lengths are sampled uniformly
    # from a window that
    #   starts with its upper limit at walk_it_max in the beginning of training, then
    # shifts down
    params["walk_it_max"] = 300
    # Width of window from which walk lengths are sampled: at any moment, new walk
    # lengths are sampled
    ...
    # Number of backprop iters until latent parameter losses (L_p_g, L_p_x, L_g) are all
    # fully weighted
    ...
    # Determine how much to use an offset for the standard deviation of the inferred
    # grounded location
    #   to reduce its influence
    params["p2g_scale_offset"] = 0
    # Additional value to offset standard deviation of inferred grounded location when
    # inferring new
    ...
    # Set how fast offset scaling should decrease - after p2g_sig_half_it +
    # p2g_sig_scale_it the offset
    ...
    # -- Model parameters
    # Decide whether to sample, or assume no noise and simply take mean of all
    # distributions
    params["do_sample"] = False
    # Decide whether to use inferred ground location while inferring new abstract
    # location, instead of
    #   only previous grounded location (James's infer_g_type)
    params["use_p_inf"] = True
    # Decide whether to use seperate grid modules that recieve shiny information for
    # object vector cells.
    #   To disable OVC, set this False, and set n_ovc to [0 for _ in
    # range(len(params['n_g_subsampled']))]
    params["separate_ovc"] = False
    # Standard deviation for initial initial g (which will then be learned)
    params["g_init_std"] = 0.5
    # Standard deviation to initialise hidden to output layer of MLP for inferring new
    # abstract location
    ...
    # Neurons for subsampled entorhinal abstract location f_g(g) for each frequency
    # module
    params["n_g_subsampled"] = [10, 10, 8, 6, 6]
    # Neurons for object vector cells. Neurons will get new modules if object vector
    # cell modules are
    #   separated; otherwise, they are added to existing abstract location modules.
    # a) No additional modules, no additional object vector neurons (e.g. when not using
    # shiny
    #   environments): [0 for _ in range(len(params['n_g_subsampled']))], and
    # separate_ovc set to False
    # b) No additional modules, but n additional object vector neurons in each grid
    # module:
    #   [n for _ in range(len(params['n_g_subsampled']))], and separate_ovc set to False
    # c) Additional separate object vector modules, with n, m neurons: [n, m], and
    # separate_ovc set to True
    params["n_ovc"] = [0 for _ in range(len(params["n_g_subsampled"]))]
    # Add neurons for object vector cells. Add new modules if object vector cells get
    # separate modules,
    ...
    # Add frequencies of object vector cell modules, if object vector cells get separate
    # modules
    params["f_initial"] = (
    ...
    # Maximum iterations of attractor dynamics per frequency in inference model, so you
    # can early
    ...
    # Maximum iterations of attractor dynamics per frequency in generative model, so you
    # can early
    ...
    # Set connections when forming Hebbian memory of grounded locations: from low
    # frequency modules to high.
    ...
    # Entry M_ij (row i, col j) is the connection FROM cell i TO cell j. Memory is
    # retrieved by
    #   h_t+1 = h_t * M, i.e. h_t+1_j = sum_i {connection from i to j * h_t_i}
    for f_from in range(params["n_f"]):
        for f_to in range(params["n_f"]):
            # For connections that involve separate object vector modules: these are
            # connected to all normal
            #   modules, but hierarchically between object vector modules
            if f_from > params["n_f_g"] or f_to > params["n_f_g"]:
                # If this is a connection between object vector modules: only allow for
                # connection from
    ...
    # During memory retrieval, hierarchical memory retrieval of grounded location is
    # implemented by early-stopping
    #   low-frequency memory updates, using a mask for updates at every retrieval
    # iteration
    ...
    for mask, max_iters in zip(
        ...
        # For each frequency, we get the number of update iterations, and insert ones in
        # the mask for those iterations
        for f, max_i in enumerate(max_iters):
            # Update masks up to maximum iteration
            for i in range(max_i):
                mask[i][n_p[f] : n_p[f + 1]] = 1.0
                # In path integration, abstract location frequency modules can influence
                # the transition of other
                #   modules hierarchically (low to high). Set for each frequency module
                # from which other frequencies
                #   input is received
    ...
    # Add connections for separate object vector cell module: only between object vector
    # cell modules - and make
    ...
    # information x with elementwise product.
    ...
    # Matrix for tiling sensory observation x to do outer product with abstract with
    # elementwise product.
    #   Also see (*) note at bottom
    params["W_tile"] = [
        torch.tensor(
    ...
    for i in range(1, min(int(comb(params["n_x_c"], 2)), params["n_x"])):
        ...
        if swap + 2 < len(code) and code[swap + 2] == 1:
            # In that case: move the second 1 all the way back - reverse everything
...
def parameter_iteration(iteration, params):
    ...
    -------
        ...
        walk_length_center : float
            Center of walk length window, within which the walk lenghts of new walks are
            uniformly sampled.
        loss_weights : torch.tensor
            Current loss weights.

    """
    # Calculate eta (rate of remembering) and lambda (rate of forgetting) for Hebbian
    # memory updates
    ...
    # Calculate center of walk length window, within which the walk lenghts of new walks
...
def generate_n_walk(params):
    """Generate number of steps to roll out before backpropagation through time
    for each iteration.
    ...
    -------
        n_walks : list
            List of number of steps to roll out before backpropagation through time for
            each iteration.
...
# W_tile and W_repeat are for calculating outer products then vector flattening by
...
#     g = g.T
#     x = x.T
#     out3 = np.matmul(np.transpose(g), x).reshape((1,4*3)) # Notice how this is not
# batch-proof!
