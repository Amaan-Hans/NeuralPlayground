...
class Model(torch.nn.Module):
    """Model class for TEM model.

    Inherits from torch.nn.Module, so it can be used as a torch module,
    and all parameters are automatically registered as trainable
    parameters.
    ...
    def forward(self, walk, prev_iter=None, prev_M=None):
        ...
        Parameters
        ----------
            walk: list of [place, observation, action] tuples, where place is a list of
            locations, observation is a one-hot
            vector of sensory observation, and action is a one-hot vector of action
            taken
            prev_iter: list of Iteration objects, which contain all variables of the
            model for each step of the previous
        ...
        Returns
        -------
            steps: list of Iteration objects, which contain all variables of the model
            for each step of the walk.

        """
        # The previous iteration may contain walks without action. These are new walks,
        # for which some parameters need to
        # be reset.
        ...
        for g, x, a in walk:
            ...
            if steps is None:
                # Use an Iteration object to set initial values before any real
                # iterations, initialising M, x_inf as zero.
            ...
            steps.append(
                Iteration(
                    g, x, a, L, M, g_gen, p_gen, x_gen, x_logits, x_inf, g_inf, p_inf
                )
            )
            # The first step is either a step from a previous walk or initialisiation
            # rubbish, so remove it
        steps = steps[1:]
    ...
    def iteration(self, x, locations, a_prev, M_prev, x_prev, g_prev):
        ...
        -------
            ...
            M: list of memory connectivity matrices for this iteration
            gt_gen: list of generated abstract (grid cell) locations for this iteration
            p_gen: list of generated place cell locations for this iteration
            x_gen: list of generated observations for this iteration
            x_logits: list of logits for generated observations for this iteration
            x_inf: list of inferred observations for this iteration
            g_inf: list of inferred abstract (grid cell) locations for this iteration
            p_inf: list of inferred grounded (place cell) locations for this iteration

        """
        # First, do the transition step, as it will be necessary for both the inference
        # and generative part of the model
        gt_gen, gt_inf = self.gen_g(a_prev, g_prev, locations)
        # Run inference model: infer grounded location p_inf (hippocampus), abstract
        # location g_inf (entorhinal).
        ...
        # *inferred* variables instead of *generated* variables (as it would when used
        # for generation)
        ...
        if self.hyper[
            ...
            # and updated separatedly if not
            M.append(
                M[0]
    ...
    def inference(self, x, locations, M_prev, x_prev, g_gen):
        ...
        Returns
        -------
            x_f: filtered sensory experience
            g: inferred abstract (grid cell) locations
            p_x: retrieved grounded (place cell) locations from memory by sensory
            experience
            p: inferred grounded (place cell) locations
        ...
        x_ = self.x2x_(x_f)
        # Retrieve grounded location from memory by doing pattern completion on current
        # sensory experience
        p_x = (
            self.attractor(
                x_, M_prev[1], retrieve_it_mask=self.hyper["p_retrieve_mask_inf"]
            )
    ...
    def generative(self, M_prev, p_inf, g_inf, g_gen):
        """Perform generative step of the TEM model.
        ...
        ----------
            ...
            g_gen: previous abstract locations

        Returns
        -------
            x_p: generated observation from inferred grounded location
            x_g: generated observation from grounded location retrieved from inferred
            abstract
            location
            x_gt: generated observation from grounded location retrieved from abstract
            location
            by transitioning
            x_p_logits: logits for generated observation from inferred grounded location
            x_g_logits: logits for generated observation from grounded location
            retrieved from
            inferred abstract location
            x_gt_logits: logits for generated observation from grounded location
            retrieved from
            abstract location by transitioning
            p_g_inf: grounded location retrieved from inferred abstract location

        """
        # Generate observation from inferred grounded location, using only the highest
        # frequency.
        # Also keep non-softmaxed logits which are used in the loss later
        x_p, x_p_logits = self.gen_x(p_inf[0])
        # Retrieve grounded location from memory by pattern completion on inferred
        # abstract
        # location
        p_g_inf = self.gen_p(g_inf, M_prev[0])  # was p_mem_gen
        # And generate observation from the grounded location retrieved from inferred
        # abstract
        # location
        x_g, x_g_logits = self.gen_x(p_g_inf[0])
        # Retreive grounded location from memory by pattern completion on abstract
        # location by
        # transitioning
    ...
    def loss(self, g_gen, p_gen, x_logits, x, g_inf, p_inf, p_inf_x):
        """Calculate losses of the current TEM iteration for each of the
        generated and inferred variable.

        Parameters
        ----------
            g_gen: previous abstract (grid cell) locations
            p_gen: previous grounded (place cell) locations
            x_logits: logits for generated observation from grounded location retrieved
            from abstract
            location by transitioning
            x: sensory observation, one-hot vector
            g_inf: inferred abstract (grid cell) locations
            p_inf: inferred grounded (place cell) locations
            p_inf_x: retrieved grounded (place cell) locations from memory by sensory
            experience
        ...
        """
        # Calculate loss function, separately for each component because you might want
        # to reweight
        # contributions later
        # L_p_gen is squared error loss between inferred grounded location and grounded
        # location retrieved
        # from inferred abstract location
        L_p_g = torch.sum(torch.stack(utils.squared_error(p_inf, p_gen), dim=0), dim=0)
        # L_p_inf is squared error loss between inferred grounded location and grounded
        # location retrieved
        ...
        # L_g is squared error loss between generated abstract location and inferred
        # abstract location
        L_g = torch.sum(torch.stack(utils.squared_error(g_inf, g_gen), dim=0), dim=0)
        # L_x is a cross-entropy loss between sensory experience and different model
        # predictions. First get true labels from the sensory part of the observation
        # only (strip any appended TD reward channel so argmax targets stay in 0..n_x-1).
        n_reward = self.hyper.get("n_td_reward", 0)
        obs_x = x[:, :-n_reward] if n_reward > 0 else x
        labels = torch.argmax(obs_x, 1)
        ...
        L_reg_g = torch.sum(
            torch.stack([torch.sum(g**2, dim=1) for g in g_inf], dim=0), dim=0
        )
        # And L_reg_p regularisation on L1 norm of p
        L_reg_p = torch.sum(
            torch.stack([torch.sum(torch.abs(p), dim=1) for p in p_inf], dim=0), dim=0
        )
    ...
    def init_trainable(self):
        ...
        This is done in a separate function so that it can be called
        again after loading a model from file.

        """
        # Scale factor in Laplacian transform for each frequency module. High frequency
        # comes first, low
        # frequency comes last. Learn inverse sigmoid instead of scale factor directly,
        # so domain of alpha is -inf, inf
        self.alpha = torch.nn.ParameterList(
            [
                torch.nn.Parameter(
                    torch.tensor(
                        np.log(
                            self.hyper["f_initial"][f]
                            / (1 - self.hyper["f_initial"][f])
                        ),
                        dtype=torch.float,
                    )
                )
        ...
        # Size matches n_x_f[0] because f_x sums place cells via W_tile back to the
        # full compressed+reward representation before decompression.
        self.b_x = torch.nn.Parameter(torch.zeros(self.hyper["n_x_f"][0]))
        # Frequency module specific scaling of sensory experience before input to
        # hippocampus
        self.w_p = torch.nn.ParameterList(
            [torch.nn.Parameter(torch.tensor(1.0)) for f in range(self.hyper["n_f"])]
        )
        # Initial activity of abstract location cells when entering a new environment,
        # like a prior on g.
                ...
                torch.nn.Parameter(
                    torch.tensor(
                        truncnorm.rvs(
                            -2,
                            2,
                            size=self.hyper["n_g"][f],
                            loc=0,
                            scale=self.hyper["g_init_std"],
                        ),
                        dtype=torch.float,
                    )
                )
        ...
        # Log of standard deviation of abstract location cells when entering a new
        ...
        # MLP for transition weights (not in paper, but recommended by James so you can
        # learn about
        ...
        # Initialise the hidden to output weights as zero, so initially you simply keep
        # the current abstract
        # location to predict the next abstract location
        self.MLP_D_a.set_weights(1, 0.0)
        # Transition weights without specifying an action for use in generative model
        # with shiny objects
        ...
        self.MLP_sigma_p = MLP(
            self.hyper["n_p"], self.hyper["n_p"], activation=[torch.tanh, torch.exp]
        )
        # MLP to generate mean of abstract location from downsampled abstract location,
        # obtained by summing
        # grounded location over sensory preferences in inference model
        self.MLP_mu_g_mem = MLP(
            self.hyper["n_g_subsampled"],
            self.hyper["n_g"],
            hidden_dim=[2 * g for g in self.hyper["n_g"]],
        )
        # Initialise weights in last layer of MLP_mu_g_mem as truncated normal for each
        # frequency module
        ...
        # MLP to generate standard deviation of abstract location from two measures
        ...
        # MLP to generate mean of abstract location directly from shiny object presence.
        # Outputs to object
        # vector cell modules if they're separated, else to all abstract location
        # modules
        self.MLP_mu_g_shiny = MLP(
            ...
            [
                n_g
                for n_g in self.hyper["n_g"][
                    (self.hyper["n_f_g"] if self.hyper["separate_ovc"] else 0) :
                ]
            ],
            hidden_dim=[
                2 * n_g
        ...
        # MLP to generate standard deviation of abstract location directly from shiny
        # object presence.
        # Outputs to object vector cell modules if they're separated, else to all
        # abstract location modules
        ...
        # MLP for decompressing highest frequency sensory experience to sensory
        # observation
        self.MLP_c_star = MLP(
            self.hyper["n_x_f"][0],
            self.hyper["n_x"],
    ...
    def init_iteration(self, g, x, a, M):
        ...
        Parameters
        ----------
            g: previous abstract (grid cell) locations
            x: sensory observation, one-hot vector
            a: previous action, one-hot vector
            M: previous memory connectivity matrix

        Returns
        -------
            iteration: Iteration object, which contains all variables of the model for
            this iteration.

        """
        # On the very first iteration, update the batch size based on the data. This is
        # useful when
        # doing analysis on the network with different batch sizes compared to training
        self.hyper["batch_size"] = x.shape[0]
        # Initalise hebbian memory connectivity matrix [M_gen, M_inf] if it wasn't
        # initialised yet
        if M is None:
            ...
            # inference
            if self.hyper["use_p_inf"]:
                # If inference and generative network share common memory: reuse same
                # connectivity, and
                # same memory vectors. Else, create a new empty memory list for
                # inference network
                M.append(
                    M[0]
                    if self.hyper["common_memory"]
    ...
    def init_walks(self, prev_iter):
        ...
        """
        # Only reset parameters for previous iteration if a previous iteration was
        # actually provided - if
        # it wasn't, all parameters will be reset when creating a fresh Iteration object
        # in init_iteration
        if prev_iter is not None:
            # The supplied previous iteration might have new walks starting, with empty
            # actions. For these
            # walks some parameters need to be reset
            for a_i, a in enumerate(prev_iter[0].a):
                # A new walk is indicated by having a None action in the previous
                # iteration
                if a is None:
    ...
    def gen_g(self, a_prev, g_prev, locations):
        """Perform transition step of the TEM model. This consists of a
        transition from previous abstract location to new abstract location
        using weights specific to action taken for each frequency module.
        ...
        """
        # Transition from previous abstract location to new abstract location using
        # weights specific to action
        # taken for each frequency module
        mu_g = self.f_mu_g_path(a_prev, g_prev)
        sigma_g = self.f_sigma_g_path(a_prev, g_prev)
        # Either sample new abstract location g or simply take the mean of distribution
        # in noiseless case.
        g = [
            (
        ...
        # But for environments with shiny objects, the transition to the new abstract
        # location shouldn't have
        #  access to the action direction in the generative model
        shiny_envs = [location["shiny"] is not None for location in locations]
        # If there are any shiny environments, the abstract locations for the generative
        # model will need to be
        # re-calculated without providing actions for those
        g_gen = (
            self.f_mu_g_path(a_prev, g_prev, no_direc=shiny_envs)
            if any(shiny_envs)
            else g
        )
    ...
    def gen_p(self, g, M_prev):
        """Generate grounded location from abstract location by pattern
        completion on abstract location. This is used in the generative model
        to generate observations from grounded location.
        ...
        """
        # We want to use g as an index for memory retrieval, but it doesn't have the
        # right dimensions (these
        # are grid cells, we need place cells). We need g_ instead
        g_ = self.g2g_(g)
        # Retreive memory: do pattern completion on abstract location to get grounded
        # location
        mu_p = self.attractor(
            g_, M_prev, retrieve_it_mask=self.hyper["p_retrieve_mask_gen"]
        )
        sigma_p = self.f_sigma_p(mu_p)
        # Either sample new grounded location p or simply take the mean of distribution
        # in noiseless case
        p = [
            (
                mu_p[f] + sigma_p[f] * np.random.randn()
                if self.hyper["do_sample"]
                else mu_p[f]
            )
            for f in range(self.hyper["n_f"])
    ...
    def gen_x(self, p):
        """Generate observation from grounded location by MLP. This is used in
        the generative model to generate observations from grounded location.
        ...
        Returns
        -------
            x: generated observation
            logits: logits for generated observation

        """
        # Get categorical distribution over observations from grounded location
        # If you actually want to sample observation, you need a reparaterisation trick
        # for categorical distributions
        # Sampling would be the correct way to do this, since observations are discrete,
        # and it's also what the
        # TEM paper says
        # However, it looks like you could also get away with using categorical
        # distribution directly as an
        # approximation of the one-hot observations
        if self.hyper["do_sample"]:
            x, logits = self.f_x(
                p
            )  # This is a placeholder! Should be done using reparameterisation trick
            # (like https://blog.evjang.com/2016/11/tutorial-categorical-variational.html)
        else:
            x, logits = self.f_x(p)
        # Return one-hot (or almost one-hot...) observation obtained from grounded
        # location, and also
        # the non-softmaxed logits
        return x, logits

    def inf_g(self, p_x, g_gen, x, locations):
        """Infer abstract location from the combination of [grounded location
        retrieved from memory by sensory experience] and [previous abstract
        location and action (path integration)]
        ...
        if self.hyper["use_p_inf"]:
            ...
            with torch.no_grad():
                ...
                # actual observation: if the memory
                # is working well, this error should be small
                n_reward = self.hyper.get("n_td_reward", 0)
                obs_x = x[:, :-n_reward] if n_reward > 0 else x
                err = utils.squared_error(obs_x, x_hat)
            # The second measure is the vector norm of the inferred abstract location;
            # good memories should have similar
            # vector norms. Concatenate the two measures as input for the abstract
            # location uncertainty function
            sigma_g_input = [
                torch.cat(
            ...
            # providing uncertainty function with
        ...
        # Infer abstract location by combining previous abstract location and grounded
        # location retrieved from memory by
        ...
        if any(shiny_envs):
            ...
            # Get abstract location for environments with shiny objects and feed to each
            # of the object vector cell modules
            ...
            # start from offset if object vector
            ...
            # inverse variance weighting to the
            # previously obtained mu and sigma - but only for object vector cell
            # modules!
            for f in range(module_start, self.hyper["n_f"]):
                # Add inferred abstract location from shiny objects to previously
                # obtained position, only for environments
                # with shiny objects
                mu, sigma = utils.inv_var_weight(
                ...
                # Either sample inferred abstract location from combined (precision
                # weighted) distribution or just take
                # mean
        g = [
            (
                mu_g[f] + sigma_g[f] * np.random.randn()
    ...
    def inf_p(self, x_, g_):
        ...
        for f in range(self.hyper["n_f"]):
            ...
            if self.hyper["do_sample"]:
                ...
                p.append(mu_p)
        # Return new memory constructed from sensory experience and inferred abstract
        # location
        return p

    def x_prev2x(self, x_prev, x_c):
        """Perform temporal filtering on sensory experience for each frequency
        module.

        Parameters
        ----------
            x_prev: previous sensory experience
            x_c: current sensory experience
    ...
    def x2x_(self, x):
        ...
        Returns
        -------
            x_: weighted and normalised sensory input to memory

        """
        # Prepare sensory input for input to memory by weighting and normalisation for
    ...
    def g2g_(self, g):
        """Prepare abstract location for input to memory by reshaping and down-
        sampling for each frequency module.
        ...
        Returns
        -------
            g_: downsampled abstract (grid cell) locations

        """
        # Prepares abstract location for input to memory by reshaping and down-sampling
        # for each frequency module
    ...
    def f_mu_g_path(self, a_prev, g_prev, no_direc=None):
        ...
        ----------
            a_prev: previous action, one-hot vector
            g_prev: previous abstract (grid cell) locations
            no_direc: list of booleans indicating for which walks the transition
        ...
        """
        # If there are no environments where the transition direction needs to be
        # omitted (e.g. no shiny objects,
        # or in inference model: set to all false
        no_direc = [False for _ in a_prev] if no_direc is None else no_direc
        # Remove all Nones from a_prev: these are walks where there was no previous
        # action, so no step needs to
        ...
        # environments where transition direction
        # needs to be omitted (can set only if any no_direc)
        for f in range(self.hyper["n_f"]):
            D_a[f][no_direc, :] = self.D_no_a[f]
        # Reshape transition weight vector into transition matrix. The number of rows in
        ...
        # Select the frequency modules of the previous abstract location that are
        # connected to each frequency module, to
        g_in = [
            torch.unsqueeze(
                torch.cat(
                    [
        ...
        # Reshape transition weight vector into transition matrix. The number of rows in
        # the transition matrix is given by the
        ...
        # Not in the paper, but recommended by James for stability: use inferred code as
        # *difference* in abstract location.
        # Calculate new abstract location from previous abstract location and difference
        g_step = [
            g + d if g.dim() > 1 else torch.unsqueeze(g + d, 0)
            for g, d in zip(g_prev, delta)
        ]
        # Not in paper, but recommended by James for stability: clamp activations
        # between -1 and 1
        g_step = self.f_g_clamp(g_step)
        # Build new abstract location from result of transition if there was one, or
        # from prior on abstract location
        # if there wasn't
        return [
            torch.stack(
                [
                    g_step[f][batch_i, :] if do_step else self.g_init[f]
                    for batch_i, do_step in enumerate(a_do_step)
                ]
            )
            for f in range(self.hyper["n_f"])
        ]
    ...
    def f_sigma_g_path(self, a_prev, g_prev):
        """Use multi layer perceptron to generate standard deviation from all
        previous abstract locations, including those that were just initialised
        and not real previous locations.
        ...
        # And take exponent to get prior sigma for the walks that didn't have a previous
        # location
        from_prior = [torch.exp(logsig) for logsig in self.logsig_g_init]
        # Now select the standard deviation generated from the previous abstract
        # location if there was one, and the prior
        # standard deviation on abstract location otherwise
        return [
            torch.stack(
                [
                    from_g[f][batch_i, :] if do_step else from_prior[f]
                    for batch_i, do_step in enumerate(a_do_step)
                ]
            )
            for f in range(self.hyper["n_f"])
    ...
    def f_mu_g_mem(self, g_downsampled):
        """Use multi layer perceptron to generate mean of abstract location
        from down-sampled abstract location, obtained by summing over sensory
        dimension of grounded location.
        ...
        Returns
        -------
            mu_g_mem: inferred abstract (grid cell) locations

        """
        return self.MLP_mu_g_mem(g_downsampled)

    def f_sigma_g_mem(self, g_downsampled):
        """Use multi layer perceptron to generate standard deviation of
        abstract location from down-sampled abstract location, obtained by
        ...
        Returns
        -------
            sigma_g_mem: standard deviation of inferred abstract (grid cell) locations

        """
        sigma = self.MLP_sigma_g_mem(g_downsampled)
        # Not in paper, but also offset this sigma over training, so you can reduce
        # influence of inferred p early on
        return [
            sigma[f] + self.hyper["p2g_scale_offset"] * self.hyper["p2g_sig_val"]
            for f in range(self.hyper["n_f"])
        ]

    def f_mu_g_shiny(self, shiny):
        """Use multi layer perceptron to generate mean of abstract location
        from boolean location shiny-ness. Outputs to object vector cell modules
        if they're separated, else to all abstract location modules.

        Parameters
        ----------
            shiny: boolean location shiny-ness
        ...
        # Then apply clamp and leaky relu to get object vector module activations, like
        # it's done for ground location
        # activations
        g = self.f_p(mu_g)
        return g
    ...
    def f_sigma_g_shiny(self, shiny):
        """Use multi layer perceptron to generate standard deviation of
        abstract location from boolean location shiny-ness. Outputs to object
        vector cell modules if they're separated, else to all abstract location
        modules.
    ...
    def f_sigma_p(self, p):
        ...
        Returns
        -------
            sigma_p: standard deviation of grounded (place cell) locations

        """
        return self.MLP_sigma_p(p)

    def f_x(self, p):
        """Use multi layer perceptron to generate observation from grounded
        location. Do this by first calculating categorical probability
        distribution over observations for a given ground location, then
        ...
        """
        # Calculate categorical probability distribution over observations for a given
        # ground location
        # p is the flattened (by concatenating rows - like reading sentences) outer
        # product of g and x (p = g^T * x).
        # Therefore to get the sensory experience x for a grounded location p, sum over
        ...
        # We'll keep both the logits (domain -inf, inf) and probabilities (domain 0, 1)
        # because both are needed later on
        probability = utils.softmax(logits)
        return probability, logits

    def f_c_star(self, compressed):
        """Use multi layer perceptron to decompress sensory experience at
        highest frequency.
        ...
        Returns
        -------
            decompressed: decompressed sensory experience

        """
        return self.MLP_c_star(compressed)

    def f_c(self, decompressed):
        """Compress sensory observation to two-hot code, then append TD reward channel.

        The input tensor has shape (batch, n_x + n_td_reward).  The first n_x columns
        are the one-hot sensory observation from the environment; the remaining
        n_td_reward columns carry the normalised TD value V(s) produced by the agent
        wrapper.  Only the sensory part goes through the two-hot table lookup; the
        reward channel is concatenated as-is so downstream memory operations can encode
        both sensory context and reward proximity.

        Parameters
        ----------
            decompressed: (batch, n_x + n_td_reward) sensory + reward observation

        Returns
        -------
            compressed: (batch, n_x_c + n_td_reward) two-hot code + reward channel

        """
        n_reward = self.hyper.get("n_td_reward", 0)
        if n_reward > 0:
            obs_part = decompressed[:, :-n_reward]
            reward_part = decompressed[:, -n_reward:]
            two_hot = torch.stack(
                [self.hyper["two_hot_table"][i] for i in torch.argmax(obs_part, dim=1)],
                dim=0,
            ).to(dtype=decompressed.dtype, device=decompressed.device)
            return torch.cat([two_hot, reward_part], dim=1)
        return torch.stack(
            [self.hyper["two_hot_table"][i] for i in torch.argmax(decompressed, dim=1)],
            dim=0,
        ).to(dtype=decompressed.dtype, device=decompressed.device)
    ...
    def f_n(self, x):
        ...
        """
        normalised = [
            utils.normalise(utils.relu(x[f] - torch.mean(x[f])))
            for f in range(self.hyper["n_f"])
        ]
        return normalised
    ...
    def f_g_clamp(self, g):
        """Calculate activation for abstract location, thresholding between -1
        and 1.
        ...
        Returns
        -------
            activation: activation for abstract (grid cell) locations
    ...
    def f_p(self, p):
        """Calculate activation for inferred grounded location, using a leaky
        relu for sparsity. Either apply to full multi-frequency grounded
        location or single frequency module.
    ...
    def attractor(self, p_query, M, retrieve_it_mask=None):
        """Retrieve grounded location from attractor network memory with
        weights M by pattern-completing query. For example, initial attractor
        input can come from abstract location (g_) or sensory experience (x_).

        Parameters
        ----------
            p_query: initial attractor input
            M: memory connectivity matrix
            retrieve_it_mask: mask for hierarchical retrieval

        Returns
        -------
            p: retrieved grounded (place cell) locations
        ...
        # Make helper list of cumulative neurons per frequency module for grounded
        # locations
        n_p = np.cumsum(np.concatenate(([0], self.hyper["n_p"])))
        # Now re-cast the grounded location into different frequency modules, since
        # memory retrieval turned it
    ...
    def hebbian(
        ...
        Returns
        -------
            M: updated memory connectivity matrix

        """
        # Create new ground memory for attractor network by setting weights to outer
        # product of learned vectors
        ...
        M_new = torch.squeeze(
            torch.matmul(
                torch.unsqueeze(p_inferred + p_generated, 2),
                torch.unsqueeze(p_inferred - p_generated, 1),
            )
        )
...
class MLP(torch.nn.Module):
    """Class for multi layer perceptron with multiple modules.

    Each module has two layers: input->hidden and hidden->output.
    ...
    def __init__(
        ...
        # Check if this network consists of module: are input and output dimensions
        # lists? If not, make them
    ...
    def set_weights(self, from_layer, value):
        """Set weights for each module from a given layer to a given value.

        Parameters
        ----------
            from_layer: layer from which to set weights
        ...
        if not isinstance(value, list):
            ...
            input_value = value
        # Run through all modules and set weights starting from requested layer to the
        # specified value
        with torch.no_grad():
            # MLP is setup as follows: w[module][layer] is Linear object,
            # w[module][layer].weight is Parameter object for
    ...
    def forward(self, data):
        """Run input data through MLP network.
        ...
        for n in range(self.N):
            ...
            # If this network doesn't consist of modules: select output from first
            # module to return
        if not self.is_list:
            output = output[0]
        # And return output
...
class LSTM(torch.nn.Module):
    ...
    def forward(self, data, prev_hidden=None):
        """Run input data through LSTM network.

        Parameters
        ----------
            data: input data
            prev_hidden: previous hidden state
    ...
    def prepare_data(self, data_in):
        ...
        # Concatenate observation and action together along column direction in each
        # step
        vectors = [
            torch.cat((step[1], action), dim=1)
            for step, action in zip(data_in, actions)
        ]
        # Then stack all these together along the second dimension, which is sequence
        # length
...
class Iteration:
    ...
    def __init__(
        ...
        """Initialise iteration with all data from one iteration of the
        model.
        """
        # Copy all inputs
        self.g = g
    ...
    def correct(self):
        """Calculate accuracy of model prediction for this iteration.
    ...
    def detach(self):
        ...
        Returns
        -------
            self: iteration with all tensors detached
