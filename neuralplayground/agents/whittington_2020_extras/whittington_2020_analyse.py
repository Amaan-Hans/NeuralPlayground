...
def performance(forward, model, environments):
    """Track prediction accuracy over walk, and calculate fraction of locations
    visited and actions taken to assess performance.

    Parameters
    ----------
        forward : list
            List of forward passes through the model, each containing the model input,
            the model output, and the model state.
        model : TEM
    ...
    -------
        all_correct : list
            List of lists of booleans, indicating for each step whether the model
            predicted the observation correctly.
        all_location_frac : list
            List of lists of floats, indicating for each step the fraction of locations
            visited.
        all_action_frac : list
            List of lists of floats, indicating for each step the fraction of actions
            taken.

    """
    # Keep track of whether model prediction were correct, as well as the fraction of
    # nodes/edges visited, across environments
    ...
    for env_i, env in enumerate(environments):
        ...
        # Not all actions are available at every location (e.g. edges of grid world).
        # Find how many actions can be taken
            ...
            for currAction in currLocation["actions"]:
                if np.sum(currAction["transition"]) > 0:
                    if model.hyper["has_static_action"]:
                        if currAction["id"] > 0:
                            action_available[
                                currLocation["id"], currAction["id"] - 1
                            ] = True
                    else:
        ...
        action_frac = []
        # Run through iterations of forward pass to check when an action is taken for
        # the first time
        for step in forward:
            ...
            correct.append(
                (
                    torch.argmax(step.x_gen[2][env_i]) == torch.argmax(step.x[env_i])
                ).cpu().numpy()
            )
        ...
        # Add performance and visitation fractions of this environment to performance
        # list across environments
...
def location_accuracy(forward, model, environments):
    """Track prediction accuracy per location, after a transition towards the
    location.
    ...
    -------
        accuracy_from : list
            List of lists of floats, indicating for each location the fraction of
            correct predictions after arriving at
            that location.
        accuracy_to : list
            List of lists of floats, indicating for each location the fraction of
            correct predictions after leaving
            that location.

    """
    # Keep track of whether model prediction were correct for each environment,
    # separated by arrival and departure location
    ...
    for env_i, env in enumerate(environments):
        ...
        accuracy_from.append(
            [
                sum(correct_from_location)
                / (len(correct_from_location) if len(correct_from_location) > 0 else 1)
                for correct_from_location in correct_from
            ]
        )
        accuracy_to.append(
            [
                sum(correct_to_location)
                / (len(correct_to_location) if len(correct_to_location) > 0 else 1)
...
def location_occupation(forward, model, environments):
    ...
    -------
        occupation : list
            List of lists of integers, indicating for each location how often it was
...
def zero_shot(forward, model, environments, include_stay_still=True):
    """Track whether the model can predict the observation correctly when it
    visits a location for the first time.
    ...
    ----------
        ...
        include_stay_still : bool
            Whether to include standing still actions in the zero-shot inference
            analysis.
    ...
    # Track for all opportunities for zero-shot inference if the predictions were
    ...
    for env_i, env in enumerate(environments):
        ...
        prev_iter = forward[0]
        # Make list that for all opportunities for zero-shot inference tracks if the
        ...
        for step in forward[1:]:
            ...
            if (
                model.hyper["has_static_action"]
                and prev_a == 0
                and not include_stay_still
            ):
                prev_a = None
            # Mark the location of the previous iteration as visited
            location_visited[prev_g] = True
            # Zero shot inference occurs when the current location was visited, but the
            # previous action wasn't taken before
            if (
                location_visited[step.g[env_i]["id"]]
                and prev_a is not None
                and not action_taken[prev_g, prev_a]
            ):
                # Find whether the prediction was correct
                correct_zero_shot.append(
                    (
                        torch.argmax(step.x_gen[2][env_i])
                        == torch.argmax(step.x[env_i])
                    ).cpu().numpy()
                )
            ...
            prev_iter = step
        # Having gone through the full forward pass for one environment, add the
        # zero-shot performance to the list of all
...
def compare_to_agents(forward, model, environments, include_stay_still=True):
    """Compare TEM performance to a 'node' and an 'edge' agent, that remember
    previous observations and guess others.
    ...
    -------
        ...
        all_correct_node : list
            List of lists of booleans, indicating for each step whether the node agent
            predicted the observation correctly.
        all_correct_edge : list
            List of lists of booleans, indicating for each step whether the edge agent
            predicted the observation correctly.

    """
    # Get the number of actions in this model
    n_actions = model.hyper["n_actions"] + model.hyper["has_static_action"]
    # Store for each environment for each step whether is was predicted correctly by the
    # model, and by a perfect node and
    # perfect edge agent
    ...
    for env_i, env in enumerate(environments):
        ...
        # And for each action in each location whether it has been taken
        action_taken = np.full((env[2], n_actions), False)
        # Make array to list whether the observation was predicted correctly or not for
        # the model
        correct_model = []
        # And the same for a node agent, that picks a random observation on first
        # encounter of a node, and the correct
        # one every next time
        correct_node = []
        # And the same for an edge agent, that picks a random observation on first
        # encounter of an edge, and the correct
        # one every next time
        correct_edge = []
        # Get the very first iteration
        prev_iter = forward[0]
        ...
        for step in forward[1:]:
            ...
            prev_a, prev_g = prev_iter.a[env_i], prev_iter.g[env_i]["id"]
            # If the previous action was standing still: only count as valid transition
            ...
            correct_model.append(
                (
                    torch.argmax(step.x_gen[2][env_i]) == torch.argmax(step.x[env_i])
                ).cpu().numpy()
            )
            # Update node agent prediction for this step: correct when this state was
            # visited beofre, otherwise chance
            correct_node.append(
                True
                if location_visited[step.g[env_i]["id"]]
                else np.random.randint(model.hyper["n_x"])
                == torch.argmax(step.x[env_i]).cpu().numpy()
            )
            # Update edge agent prediction for this step: always correct if no action
            # taken, correct when action leading
            ...
            # And update the previous iteration to the current iteration
            prev_iter = step
        # Add the performance of model, node agent, and edge agent for this environment
        # to list across environments
...
def rate_map(forward, model, environments):
    """Calculate the firing rate of each cell in the model for each location in
    each environment.
    ...
    -------
        all_g : list
            List of lists of lists of floats, indicating for each frequency module, for
    ...
    """
    # Store location x cell firing rate matrix for abstract and grounded location
    # representation across environments
    all_g, all_p = [], []
    # Go through environments and collect firing rates in each
    for env_i, env in enumerate(environments):
        # Collect grounded location/hippocampal/place cell representation during walk:
        # separate into frequency
        # modules, then locations
        p = [[[] for loc in range(env[2])] for f in range(model.hyper["n_f"])]
        # Collect abstract location/entorhinal/grid cell representation during walk:
        # separate into frequency
        # modules, then locations
        ...
        for step in forward:
            # Run through frequency modules and append the firing rates to the correct
            # location list
            for f in range(model.hyper["n_f"]):
                g[f][step.g[env_i]["id"]].append(step.g_inf[f][env_i].detach().cpu().numpy())
                p[f][step.g[env_i]["id"]].append(step.p_inf[f][env_i].detach().cpu().numpy())
        # Now average across location visits to get a single represenation vector for
        # each location for each frequency
        for cells, n_cells in zip([p, g], [model.hyper["n_p"], model.hyper["n_g"]]):
            for f, frequency in enumerate(cells):
                # Average across visits of the each location, but only the second half
                # of the visits so model
                # roughly know the environment
                for i, location in enumerate(frequency):
                    frequency[i] = (
                        sum(location[int(len(location) / 2) :])
                        / len(location[int(len(location) / 2) :])
                        if len(location[int(len(location) / 2) :]) > 0
                        else np.zeros(n_cells[f])
                    )
                # Then concatenate the locations to get a [locations x cells for this
                # frequency] matrix
                cells[f] = np.stack(frequency, axis=0)
        # Append the final average representations of this environment to the list of
        # representations across environments
        all_g.append(g)
        all_p.append(p)
    # Return list of locations x cells matrix of firing rates for each frequency module
...
def generate_input(environment, walk):
    ...
    ----------
        ...
        walk : list
            List of lists of lists, indicating for each step the location, observation,
            and action.
    ...
    if walk is None:
        # Generate a single walk from environment with length depending on number of
        # locations (so you're
        # likely to visit each location)
        walk = environment.generate_walks(environment.graph["n_locations"] * 100, 1)[0]
        # Now this walk needs to be adjusted so that it looks like a batch with batch
        # size 1
        for step in walk:
